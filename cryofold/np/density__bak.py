...
class DensityInfo:
    ...
    # def get_size(self):
    #     return 
    def get_linkage_matrix0(self, sb=None, points=None, dist=None, thr=4.5, verbose=1):
        ...
        prob = []
        if sb is not None:
            points = sb.get_ca_coords()
        if dist is None:  
            dist = torch.cdist(points, points) 
        
        n = len(points)
        dist = dist + torch.eye(n, device=self.device)*(thr+1)

        hist = []
        link_mat = torch.zeros_like(dist, device=self.device)
        # for i in tqdm(range(n)):
        for i in range(n):
            ind = torch.nonzero(dist[i,:]<=thr)[:,0]
            for j in ind:
                j = j.item()
                if (i,j) in hist or (j,i) in hist:
                    continue
                else:
                    hist.append((i,j))
                    hist.append((j,i))
                ...
                if verbose>=1:
                    if sb is not None:
                        gt = int(sb.resnums[j]-sb.resnums[i]<=1)
                    else:
                        gt = int(j-i<=1)
                dd = self.get_density_between(points[i], points[j])
                if verbose>=2:
                    print(dd)
                ...
                if len(dd)>0:
                    ...
                    mi,ind = dd.min(0)
                    me = torch.stack((dd[0],dd[-1]))
                    lo = (mi>=(me.mean() - me.std())).long()
                    if lo==0 and (mi>self.meanp1s).long() ==1:
                        gl=1
                    else:
                        gl=lo
                    pb = mi/(me.mean() - me.std())
                    if pb>=1.0 or ind==0 or ind==len(dd)-1:
                        pb= 1.0
                    else:
                        pb = (1/(1+1000*torch.exp(-12*pb))).item()
                else:
                    gl = 0
                    pb = 0.0
                ...
                if verbose>=1:
                    gtgt.append( gt)
                    acc0.append( int(gt==gl))
                    prob.append( pb)
                if verbose>=2:
                    print("{} {} {} {} {:.2f} {:>6.3f} {:>6.3f}  {:>6.3f} ".format(
                        sb.resnums[i], 
                        sb.resnums[j], 
                        gt,
                        gl,
        ...
        if verbose>=1:
            pos = np.array(gtgt) * np.array(prob)
            neg = (1-np.array(gtgt)) * np.array(prob)
            print( "acc: {:.4f} pos: {:.4f} {:d} neg: {:.4f} {:d}".format(
                np.array(acc0).mean(), 
                pos[pos.nonzero()].mean(),  
                np.array(gtgt).sum(),
                neg[neg.nonzero()].mean(),
                (1-np.array(gtgt)).sum(),
                neg[neg.nonzero()].mean(),
                )
            )
    ...
    def get_linkage_matrix(self, sb=None, points=None, dist=None, thr=4.5, verbose=1):
        # from tqdm import tqdm
        gtgt = []
        acc0 = []
        acc1 = []
    ...
    def get_density_between(self, p0, p1):
        def diag_ps(a, e):
            val, ind = torch.max(torch.abs(e - a), 0)
            if val ==0:
                return torch.stack([a]).long()
            elif val ==1:
                return torch.stack([a, e]).long()
            else:
                # print(a_i[ind], e_i[ind]+1)
                if e[ind]+1 - a[ind] > 0:
                    s = 1
                else:
                    s = -1 
                x = torch.arange(a[ind], e[ind]+s, s).to(a.device)
                t = (x - a[ind]) / val
                # import pdb;pdb.set_trace()
                return torch.round(( s * t.unsqueeze(dim=1)*(e - a).unsqueeze(dim=0) + a)).long()
        dp0 = torch.round(p0/self.apix - self.offset)#.long()
        dp1 = torch.round(p1/self.apix - self.offset)#.long()
        # dp0, dp1 = torch.tensor([0.0,0.0,0.0,]), torch.tensor([3.0,3.0,3.0,])
        dp = diag_ps(dp0, dp1)#.reshape(1,3)
        # import pdb;pdb.set_trace()
        dd = []
        # print(dp0,dp1)
        # print(dp)
        for i in range(dp.shape[0]):
            d = self.get_density_at(dp[i])
            # if d is None or (i==0 or i==dp.shape[0]-1) and d < self.mean:
            if d is None :
                continue
            dd.append(d)
        return torch.tensor(dd, device=self.device)
        # return dd
    ...
    def cos_sim(self, mol_den):
        ...
        # t_ov, m_ov = self.overlap(mol_den)
