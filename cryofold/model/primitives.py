...
from cryofold.utils.checkpointing import get_checkpoint_fn
from cryofold.utils.chunk_utils import _chunk_slice
from cryofold.utils.kernel import attention_core
from cryofold.utils.kernel import LayerNorm
...
class MLP3D(nn.Module):
    ...
    def __init__(self, input_dim=2048, embed_dim=768):
        super().__init__()
        self.proj = Linear(input_dim, embed_dim)
...
class MLP(nn.Module):
    ...
    def __init__(self, input_dim, hidden_dim, output_dim, num_layers=3, dtype=None):
        super().__init__()
...
class TransposeConvUpsampling(nn.Module):
    ...
    def __init__(self, in_channels=None, out_channels=None, kernel_size=3, scale_factor=(2, 2, 2)):
        super(TransposeConvUpsampling,self).__init__()
...
class LayerNorm3d(torch.nn.Module):
    """
    LayerNorm3d where the batch statistics and the affine parameters are fixed.
    ...
    def forward(self, x):
        ...
        x = self.norm(x)
        x = x.permute(0, 4, 1, 2, 3).contiguous() # (N, L, H, W, C) -> (N, C, L, H, W)
        return x   
...
class NestedTensor(object):
    ...
    def sub(self, i=0, s=1):
        assert i < self.tensors.shape[1], "NestedTensor size error."
        return NestedTensor(self.tensors[:,i:i+s], self.mask)

    def reshape_for_infer(self):
        s = self.tensors.shape
        return NestedTensor(self.tensors.view(s[1],s[0],s[2],s[3],s[4],), self.mask)
