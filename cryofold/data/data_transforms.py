...
from cryofold.config import NUM_RES, NUM_EXTRA_SEQ, NUM_TEMPLATES, NUM_MSA_SEQ
from cryofold.np import residue_constants as rc
...
def make_atom14_positions(protein):
    ...
    alternative_gt_positions = torch.einsum(
        "...rac,...rab->...rbc", residx_atom14_gt_positions, renaming_transform.to(residx_atom14_gt_positions)
    )
    ...
    alternative_gt_mask = torch.einsum(
        "...ra,...rab->...rb", residx_atom14_gt_mask, renaming_transform.to(residx_atom14_gt_mask)
    )
    ...
    N = len(protein["aatype"])
    ind = np.tile(residx_atom14_to_atom37.reshape(N,14,1),(1,1,3)) 
    def local_atom37_to_atom14(coords_, masks_):
        coords14      = np.take_along_axis(coords_, ind, axis=1)
        masks_atom14  = np.take_along_axis(masks_, residx_atom14_mask.numpy().astype(np.int32), axis=1)
        masks14_      = masks_atom14 * residx_atom14_mask
        coords14_masks = np.tile(masks14_.reshape(N,14,1),(1,1,3)) 
        coords14 = coords14 * coords14_masks
        return coords14, coords14_masks

    coords14, masks14 = local_atom37_to_atom14(protein["all_atom_positions"], protein["all_atom_mask"])
    # import pdb;pdb.set_trace()
    protein["all_atom_positions_14"] = coords14
    protein["all_atom_mask_14"] = torch.from_numpy(masks14)
