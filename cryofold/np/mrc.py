...
class MRCHeader:
    ...
    @classmethod
    def make_default_header(cls, data, is_vol=True, Apix=1., nxyzstart=(0,0,0)):
        nz, ny, nx = data.shape
        ...
        label[2] = "start: {:.6f}, {:.6f}, {:.6f}".format(nxyzstart[0], nxyzstart[1], nxyzstart[2])
        ...
        vals = [nx, ny, nz,
                ...
                nxyzstart[0], nxyzstart[1], nxyzstart[2], # nxstart, nystart, nzstart
                nx, ny, nz, # mx, my, mz
...
def write(fname, array, header=None, Apix=1., nxyzstart=(0,0,0), is_vol=None):
    # get a default header
    nxyzstart = [int(nxyzstart[0]),int(nxyzstart[1]),int(nxyzstart[2])]
    if header is None:
        if is_vol is None:
            is_vol = True if len(array.shape) == 3 else False # Guess whether data is vol or image stack
        header = MRCHeader.make_default_header(array, is_vol, Apix, nxyzstart)
    # write the header
    f = open(fname,'wb')
    header.write(f)
    f.write(array.tobytes())
