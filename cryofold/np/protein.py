...
def from_prediction(
    ...
    atom_positions=copy.deepcopy(result["final_atom_positions"])
    atom_mask=result["final_atom_mask"]
    residue_index=features["residue_index"] + 1
    b_factors=b_factors
    if len(features["aatype"].shape) == 2:
        aatype = aatype[0]
    ...
    if save_type == "backbone_frame":
        # import pdb;pdb.set_trace()
        atom_positions[:,1,:] = copy.deepcopy(result['backbone_frame'])[-1,:,4:]
        atom_mask = np.zeros_like(atom_mask)
        atom_mask[:,1]=1
    return Protein(
        aatype=aatype,
        atom_positions=atom_positions,
