...
from cryofold.utils import feats
from cryofold.utils.rigid_utils import Rotation, Rigid
...
def experimentally_resolved_loss(
    ...
    **kwargs,
    ...
    # import pdb;pdb.set_trace()
    loss = torch.sum(loss, dim=-1) / (eps + torch.sum(atom37_atom_exists, dim=(-1, -2)))
...
def amino_acid_loss(logits, aatype, seq_mask, num_classes = 21, eps=1e-8, **kwargs):
    """
    Computes BERT-style masked MSA loss. Implements subsection 1.9.9.

    Args:
        logits: [N_dec ,*, N_res, 21] predicted aa distribution
        aatype: [*, N_res] true aa
        seq_mask: [*, N_res] seq mask
    Returns:
        amino acid loss
    """
    def single_dec(logits_dec):
        errors = softmax_cross_entropy(
            logits_dec, torch.nn.functional.one_hot(aatype.long(), num_classes=num_classes)
        )
        loss = errors * seq_mask
        return loss
    multipe_dec = vmap(single_dec)
    losses = multipe_dec(logits)
...
def secondary_structure_loss(logits, sstype, seq_mask, num_classes = 4, eps=1e-8, **kwargs):
    """
    Computes BERT-style masked MSA loss. Implements subsection 1.9.9.

    Args:
        logits: [N_dec ,*, N_res, 4] predicted ss distribution
        aatype: [*, N_res] true ss
        seq_mask: [*, N_res] seq mask
    Returns:
        secondary structure loss
    """
    # if len(logits.shape) == 4:
    #     sstype   = sstype[None,:,:].repeat([logits.shape[0],1,1])
    #     seq_mask = seq_mask[None,:,:].repeat([logits.shape[0],1,1])
    # errors = softmax_cross_entropy(
    #     logits, torch.nn.functional.one_hot(sstype.long(), num_classes=4)
    # )
    
    # # loss = errors * all_atom_mask[:,:,1]
    # loss = errors * seq_mask
    # loss = torch.mean(loss)
    # import pdb;pdb.set_trace()
    def single_dec(logits_dec):
        errors = softmax_cross_entropy(
            logits_dec, torch.nn.functional.one_hot(sstype.long(), num_classes=num_classes)
        )
        loss = errors * seq_mask
        return loss
    multipe_dec = vmap(single_dec)
...
def get_pred_match(indices):
    # import pdb;pdb.set_trace()
    batch_idx = torch.cat([torch.full_like(src, i) for i, (src, _) in enumerate(indices)])
    src_idx = torch.cat([src for (src, _) in indices])

    # batch_idx = torch.stack([torch.full_like(src, i) for i, (src, _) in enumerate(indices)])
    # src_idx = torch.stack([src for (src, _) in indices])
    return batch_idx, src_idx

def amino_acid_match_loss(logits, indices, aatype, atom14_gt_exists, num_classes =20, eps=1e-8, **kwargs):
    """

    Args:
        logits: [*, bs, N_seq, 24] predicted aa distribution
        aatype: [*, bs, N_seq, N_res] true ss
        indices: 
    Returns:
        amino acid match loss
    """
    N_dec, N_bs = logits.shape[:2]
    # seq_mask = seq_mask.bool()
    seq_mask = atom14_gt_exists[...,1].bool()
    # import pdb;pdb.set_trace()

    tgt_cls = [aatype[i][seq_mask[i]] for i in range(N_bs) ]
    def single_dec(i):
        logits_dec = logits[i]
        bs, num_queries = logits_dec.shape[:2]
        idx = get_pred_match(indices[i])
        # import pdb;pdb.set_trace()

        tgt_cls_o = torch.cat([t[J] for t, (_, J) in zip(tgt_cls, indices[i])])
        true_cls = torch.full(
            logits_dec.shape[:2], num_classes, dtype=torch.int64, device=logits_dec.device)
        true_cls[idx] = tgt_cls_o
        # import pdb;pdb.set_trace()

        loss = F.cross_entropy(logits_dec.transpose(1, 2), true_cls)
        return loss
    losses = []
    for i in range(N_dec):
        losses.append(single_dec(i))
    # import pdb;pdb.set_trace()
...
def secondary_structure_match_loss(logits, indices, sstype, atom14_gt_exists, num_classes = 3, eps=1e-8, **kwargs):
    """

    Args:
        logits: [*, bs, N_seq, 4] predicted aa distribution
        sstype: [*, bs, N_seq, N_res] true ss
        indices: 
    Returns:
        secondary structure match loss
    """
    N_dec, N_bs = logits.shape[:2]
    # seq_mask = seq_mask.bool()
    seq_mask = atom14_gt_exists[...,1].bool()

    tgt_cls = [sstype[i][seq_mask[i]] for i in range(N_bs) ]
    def single_dec(i):
        logits_dec = logits[i]
        bs, num_queries = logits_dec.shape[:2]
        idx = get_pred_match(indices[i])
        tgt_cls_o = torch.cat(
            [t[J] for t, (_, J) in zip(tgt_cls, indices[i])])
        true_cls = torch.full(
            logits_dec.shape[:2], num_classes, dtype=torch.int64, device=logits_dec.device)
        true_cls[idx] = tgt_cls_o
...
def normed_ca_match_loss(positions_ca, normed_ca_positions, indices, atom14_gt_exists, eps=1e-8, **kwargs):
    """
    ...
    Returns:
        amino acid match loss
    """
    N_dec, N_bs = positions_ca.shape[:2]
    # seq_mask = seq_mask.bool()
    seq_mask = atom14_gt_exists[...,1].bool()

    tgt_ca = [normed_ca_positions[i][seq_mask[i]] for i in range(N_bs) ]
    # import pdb;pdb.set_trace()

    def single_dec(i):
        pred_ca = positions_ca[i]
        bs, num_res = pred_ca.shape[:2]

        idx = get_pred_match(indices[i])
        prd_ca_ = pred_ca[idx]

        tgt_ca_ = torch.cat([t[J] for t, (_, J) in zip(tgt_ca, indices[i])])

        loss = F.l1_loss(prd_ca_, tgt_ca_)
        return loss
    losses = []
    for i in range(N_dec):
        losses.append(single_dec(i))
...
def cryoem_segmentation_loss(logits, cryoem_seglabel, cryoem_mask, num_classes = 24, eps=1e-8, **kwargs):
    """
    Computes cryoem segmentation loss. Implements subsection 1.9.9.

    Args:
        logits: [*, c_out, w, h, l] predicted cryoem aa class distribution
        cryoem_seglabel: [*, 1, w, h, l] true cryoem aa class
        cryoem_mask: [*, 1, w, h, l] cryoem mask
    Returns:
        cryoem segmentation loss
    """

    logits   = logits.permute(0, 2, 3, 4, 1).flatten(1,3)
    true_den = cryoem_seglabel.flatten(1,4).long()
    mask_den = cryoem_mask.flatten(1,4).long()

    errors = softmax_cross_entropy(
        logits, torch.nn.functional.one_hot(true_den, num_classes=num_classes)
    )
    loss = torch.sum(errors * mask_den, dim=-1) / (
        eps + torch.sum(mask_den, dim=-1)
    )
...
def backbone_frame_loss(
    backbone_frame: torch.Tensor,
    backbone_rigid_tensor: torch.Tensor,
    backbone_rigid_mask: torch.Tensor,
    use_clamped_fape: Optional[torch.Tensor] = None,
    clamp_distance = 10.0,
    ...
    **kwargs,
    ...
    # Average over the batch dimension
    loss = torch.mean(bf_loss)
    
    return loss


def normed_ca_loss(
    positions_ca: torch.Tensor,
    atom14_gt_exists,
    normed_ca_positions,
    use_clamped_fape: Optional[torch.Tensor] = None,
    clamp_distance = 10.0,
    ...
    **kwargs,
    ...
    # print("positions_ca: ", positions_ca.shape)
    # import pdb;pdb.set_trace()
    def loss_pos_l1(pred_positions, atom_mask):
        # error = F.smooth_l1_loss(src, normed_ca_positions, reduction='none')
        error = F.l1_loss(pred_positions, normed_ca_positions, reduction='none')
        loss = torch.sum(error * atom_mask, dim=(-2, -1)) / (1e-6 + torch.sum(atom_mask, dim=(-2, -1)))
        return loss
    if len(positions_ca.shape)==4:
        atom_mask = atom14_gt_exists[0,:,:3]
        N_dec = positions_ca.shape[0]

        # structure module Ca
        losses = []
        for i in range(N_dec):
            losses.append(loss_pos_l1(positions_ca[i], atom_mask))
        # import pdb;pdb.set_trace()
        loss = torch.mean(torch.stack(losses))
    else:
        atom_mask = atom14_gt_exists[...,:3]
        losses = loss_pos_l1(positions_ca, atom_mask)
        loss = torch.mean(losses)
...
class CryoFoldLoss(nn.Module):
    """Aggregation of the various losses described in the supplement"""
    def __init__(self, config):
        super(CryoFoldLoss, self).__init__()
        self.config = config
    ...
    def forward(self, out, batch, _return_breakdown=False):
        ...
        if(self.config.structure_module.enabled):
            ...
            loss_fns["plddt"] = lambda: lddt_loss(
            ...
            loss_fns["vio"] = lambda: violation_loss(
                out["violation"],
                **batch,
            )
            loss_fns["chi"] = lambda: supervised_chi_loss(
                out["sm"]["angles"],
        ...
        # print(batch.keys())
        # print("sstype" in batch)
        # import pdb;pdb.set_trace()

        if(self.config.amino_acid.enabled):
            loss_fns["aa"] = lambda: amino_acid_loss(
                logits=out["amino_acid_logits"],
                **{**batch, **out, **self.config.amino_acid},
            )

        if(self.config.amino_acid.enabled and self.config.hungarian_matcher.enabled):
            loss_fns["aam"] = lambda: amino_acid_match_loss(
                logits=out["amino_acid_logits"], indices=out['hungarian_matches'],
                **{**batch, **out, **self.config.amino_acid},
            )
            

        if(self.config.secondary_structure.enabled):
            loss_fns["ss"] = lambda: secondary_structure_loss(
                logits=out["secondary_structure_logits"],
                **{**batch, **out, **self.config.secondary_structure},
            )
        
        if(self.config.secondary_structure.enabled and self.config.hungarian_matcher.enabled):
            loss_fns["ssm"] = lambda: secondary_structure_match_loss(
                logits=out["secondary_structure_logits"], indices=out['hungarian_matches'], 
                **{**batch, **out, **self.config.secondary_structure},
            )
        
        if(self.config.backbone_frame.enabled):
            loss_fns["bf"] = lambda: backbone_frame_loss(
                backbone_frame=out["backbone_frame"],
                **{**batch, **self.config.backbone_frame},
            )
            loss_fns["nci"] = lambda: normed_ca_loss(
                positions_ca=out["normed_backbone_frame"][:,:,:,4:],
                **{**batch, **self.config.normed_ca_init},
            )
            # import pdb;pdb.set_trace()
            if "final_normed_ca_positions" in out:
                loss_fns["ncf"] = lambda: normed_ca_loss(
                    positions_ca=out["final_normed_ca_positions"],
                    **{**batch, **self.config.normed_ca_final},
                )
        
        if(self.config.backbone_frame.enabled and self.config.hungarian_matcher.enabled):
            loss_fns["ncim"] = lambda: normed_ca_match_loss(
                positions_ca=out["normed_backbone_frame"][:,:,:,4:],
                indices=out['hungarian_matches'], 
                **{**batch, **self.config.normed_ca_init},
            )

        if(self.config.cryoem_segmentation.enabled):
            loss_fns["cs"] = lambda: cryoem_segmentation_loss(
                logits=out["cryoem_segmentation_logits"],
                **{**batch, **out, **self.config.cryoem_segmentation},
            )

            
        
        loss_weight_name_dict = {
            "plddt": "plddt_loss",
            "dist": "distogram",
            "fape": "fape",
            "msa":  "masked_msa",
            "chi":  "supervised_chi",
            "vio":  "violation",
            "exp":  "experimentally_resolved",
            "aa":   "amino_acid",
            "aam":  "amino_acid",
            "ss":   "secondary_structure",
            "ssm":  "secondary_structure",
            "bf":   "backbone_frame",
            "ncim": "normed_ca_init",
            "nci":  "normed_ca_init",
            "ncf":  "normed_ca_final",
            "cs":   "cryoem_segmentation",
        }

        cum_loss = 0.
        losses = {}
        for loss_name, loss_fn in loss_fns.items():
            loss_name_cfg = loss_weight_name_dict[loss_name]
            weight = self.config[loss_name_cfg].weight
            loss = loss_fn()
            # print(loss_name_cfg, loss)
            # import pdb;pdb.set_trace()
        ...
        # unscaled loss
        losses["us_loss"] = cum_loss.detach().clone()
